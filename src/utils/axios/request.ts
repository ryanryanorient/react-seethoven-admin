...
export class ApiRequest {
  ...
  // constructor(config: AxiosRequestConfig) { //替换为扩展后的ApiRequestConfig
  ...
  // 针对单个请求的处理、拦截
  // request(config: AxiosRequestConfig) {//为了添加对单个请求的拦截 使用扩展了的 ApiRequestConfig 替换 AxiosRequestConfig
  ...
  get<T>(config: ApiRequestConfig<T>): Promise<T> {
    //需要给ApiRequestConfig传入T类型 否则是默认AxiosResponse类型 与实际不匹配
    return this.request<T>({ ...config, method: 'GET' })
  }
  post<T>(config: ApiRequestConfig<T>): Promise<T> {    
    return this.request<T>({ ...config, method: 'POST' })
  }
  put<T>(config: ApiRequestConfig<T>): Promise<T> {
